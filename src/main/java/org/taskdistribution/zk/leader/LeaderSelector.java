...
public class LeaderSelector implements Closeable {
     ...
     */
    public void start() {
        Preconditions.checkState(state.compareAndSet(LeaderSelector.State.LATENT, LeaderSelector.State.STARTED), "Cannot be started more than once");
    ...
    public boolean requeue() {
        Preconditions.checkState(state.get() == LeaderSelector.State.STARTED, "close() has already been called");
        return internalRequeue();
    }

    private synchronized boolean internalRequeue() {
        if (!isQueued && (state.get() == LeaderSelector.State.STARTED)) {
            isQueued = true;
    ...
    public synchronized void close() {
        Preconditions.checkState(state.compareAndSet(LeaderSelector.State.STARTED, LeaderSelector.State.CLOSED), "Already closed or has not been started");
